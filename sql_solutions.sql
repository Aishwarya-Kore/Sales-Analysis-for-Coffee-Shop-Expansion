...
-- Used Dataset from Kaggle : Monday Coffee SQL Data Analysis Project
...
SELECT city_name,
	ROUND((population * 0.25)/1000000, 2) as coffee_consumersNO
FROM city
ORDER BY coffee_consumersNO DESC;
...
(
	SELECT 
		ci.city_name,
		p.product_name,
		COUNT(s.sale_id) as ci,city_name,
	...
	-- ORDER BY ci.city_name, ci.city_name DESC
) as t1
WHERE ranks <= 3;
